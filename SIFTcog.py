...
class targetModel():
    def __init__(self, nbPointMax):
        ...
        self.sift = cv2.xfeatures2d.SIFT_create(nfeatures=nbPointMax)
    ...
    def createModel(self, img, mask, imgCenter):
        ...
        kp, des = self.sift.detectAndCompute(self.targetImage, None)
...
def matchModel(targetModel, currentImg, nbPointMax, LoweCoeff):
    ...
    sift = cv2.xfeatures2d.SIFT_create(nfeatures=nbPointMax)
    matcher = cv2.DescriptorMatcher_create("BruteForce-L1")
    # compute ORB keypoint and descriptor
    kp, des = sift.detectAndCompute(currentImg, None)
    # draw keypoint
...
model = targetModel(nbPointMax=500)
